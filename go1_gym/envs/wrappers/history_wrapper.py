...
class HistoryWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # Handle both 4-value and 5-value returns from env.step()
        step_result = self.env.step(action)
        if len(step_result) == 5:
            # Base class returns 5 values
            obs, privileged_obs, rew, done, info = step_result
        elif len(step_result) == 4:
            # Wrapped returns 4 values with privileged_obs in info
            obs, rew, done, info = step_result
            privileged_obs = info.get("privileged_obs", None)
        else:
